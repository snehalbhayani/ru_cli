...
pub fn insert_command(conn: &Connection, command: &str, frequency:i32) {
    ...
    conn.execute(
        "INSERT INTO commands (command, timestamp, frequency) VALUES (?1, ?2, ?3)",
...
pub fn get_full_history(conn: &Connection) ->  Result<Vec<String>> {
    let mut stmt = conn.prepare("SELECT timestamp, command FROM commands ORDER BY timestamp DESC LIMIT 100")?;
...
pub fn get_last_n_commands(conn: &Connection, n: i32) {
    let mut stmt = conn.prepare("SELECT command FROM commands ORDER BY timestamp DESC LIMIT ?1").unwrap();
